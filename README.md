...
|------|-------------|----------------|-------------|---------------|-----------------|
| **理论基础** | 多维市场温度计 | Stan Weinstein 四阶段 | Mark Minervini VCP | 技术抄底（均值回归） | Elder Impulse + 多维检查 |
...
```bash
# 一键运行（推荐）— v2.1 增量模式自动完成价格拉取 + 策略计算 + Market Pulse
...
```bash
# 方式一：一键执行（推荐，v2.1 自动联动）
python scripts/save_prices_yfinance.py
# 自动完成：增量拉取价格 → 策略计算 → Market Pulse 更新

# 方式二：分步手动执行（适合调试或单独运行某策略）
...
- **方式一**：`save_prices_yfinance.py` v2.1 完成价格拉取后会自动运行策略计算和 Market Pulse 更新，无需手动运行后续步骤
- **方式二**：Step 1 失败不影响后续步骤（Market Pulse 会优雅降级）
...
基于 FastAPI + Jinja2 SSR + Chart.js 的本地 Web Dashboard，提供三个核心页面，复用 Phase 1-3 的全部 `lib/db.py` 查询 API，零数据库改造。Phase 6 新增：国际化支持（中/英双语）+ 技术分析四标签面板 + 四策略状态卡片。
...
| 页面 | 路径 | 功能 |
|------|------|------|
| **Dashboard** | `/` | Market Pulse 状态 + 四策略信号摘要（Stage 2 / VCP / Bottom Fisher / Buying Checklist）+ 近期信号变化 + 30天走势图 |
